...
class RedisWorkerQueue(WorkerQueue):
    def __init__(self, redis_url: str, pending_key: str = "llm:jobs:pending", failed_key: str = "llm:jobs:failed") -> None:
    ...
    async def fail(self, job_id: str, reason: str) -> None:
        await self._redis.rpush(self._failed_key, json.dumps({"job_id": job_id, "reason": reason}))
        
    async def push(self, job: WorkerJob) -> None:
        await self._redis.rpush(self._pending_key, json.dumps({"job_id": job.job_id, "prompt": job.prompt}))
